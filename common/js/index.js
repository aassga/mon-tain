window.onload = function () {
  ...
  // var apiDomain = 'https://testfapi.52buddha.com';//測試機路徑
  var apiDomain = 'https://fapi.tienmama.com';//正式機路徑
  ...
  localStorage.getItem('UUID') === null ? localStorage.setItem('UUID', UUID()) : localStorage.getItem('UUID')
  ...
  function getLoginInfo() {
    ...
    axios.post(apiDomain + '/h5free/member/ipLogin', {
      actId: actId,
      code: localStorage.getItem('UUID') 
    }).then((res) => {
      if (res && res.data.code === 200) {
        config = {headers: {Authorization: res.data.data.tokenHead + res.data.data.token}};
        my(); // 重設用戶資訊
      }
      loadingCount--; checkLoading();
    }).catch((err)=> {
  ...
  /* mode = true 更新數據 */
  function my(mode) {
    axios.post(apiDomain + '/h5/member/my?actId=' + actId, {}, config).then((res) => {
      if (res && res.data.code === 200) {
        drawTimesBalance = res.data.data.drawTimesBalance;
        drawTimesDone = res.data.data.drawTimesDone;
        emailStatus = res.data.data.emailStatus;
        friendStatus = res.data.data.friendStatus;
        memberId = res.data.data.memberId;
        todayShareCanGetTimes = res.data.data.todayShareCanGetTimes;
        unFinishPrizeId = res.data.data.unFinishPrizeId;
        ...
        if (drawTimesBalance < 1) {
          ...
          $('#click-game-area').css({'cursor':'not-allowed'})
        } else {
          $('.limit-tips-text').text('繼續抽獎');
          $('.table-count-no-login').show();
          $('.table-count-login').hide();
          $('#click-game-area').css({'cursor':'pointer'})
        }
        ...
        if (!mode) {
          ...
          if (unFinishPrizeId && unFinishPrizeId > 0) {
            ...
            $('.unFinishprizeTime span').text(res.data.data.prizeTime);
  ...
  function drawRouletteWheel() {
    ...
    if (canvas.getContext) {
      ...
      for (var i = 0; i < prizes.length; i++) {
        ...
        ctx.drawImage(img[0], -50, 10, 70* ratio, 70* ratio);
        ctx.restore();
      }
    }
  }
  ...
  /* 獲取獎項列表 */
  loadingCount++; checkLoading();
  axios.get(apiDomain + '/h5free/prize/list?source=tien').then((res)=> {
    /* 活動編號 */
  ...
  /* ----------------------------------------- 點擊以後的效果 ----------------------------------------- */
  $('#click-game-area').on('click', function () {
    // if (!isLogin) {
    //   // loginThirdParty();
    ...
    // } else if (canRun && drawTimesDone <= 0) {
    //   /* 沒有抽獎次數 */
    //   $('#pop-zero').modal('show');
    // } else
    if (canRun && drawTimesBalance > 0) {
      ...
      axios.get(apiDomain + '/h5/prize/run/' + actId, config).then((res) => {
  ...
  $('.submit-info').on('click', function () {
    ...
    axios.post(apiDomain + '/h5/prize/contactInfo', {
      ...
      if (res.data.code === 200) {
        ...
        $('#pop-address .pop-address-tips').show();
        $('#pop-address .pop-address-tips').text(res.data.message);
      }
      loadingCount--; checkLoading();
    }).catch((err) => {
      $('#pop-maintain').modal('show');
  ...
  $("#counties").on('change', function () {
    ...
    cities_object[this.value].forEach((el)=> {
